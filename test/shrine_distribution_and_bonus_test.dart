...
void main() {
  ...
  group('Kutlu Tapınak Dağılımı ve Mesafe Testleri (11 Tapınak, Min Mesafe >= 7)', () {
    ...
    test('Her iki Kutlu Tapınak arasındaki mesafe en az 5 karo olmalıdır', () {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      for (int i = 0; i < shrineTiles.length; i++) {
        for (int j = i + 1; j < shrineTiles.length; j++) {
          ...
          expect(distance >= 5, isTrue,
              reason:
                  'Tapınak ${tileA.coord} ile ${tileB.coord} arasındaki mesafe $distance, asgari 5 olmalıdır.');
        }
      }
    });
