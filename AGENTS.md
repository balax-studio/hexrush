...
27. **Kesin FPS Koruma, Sıfır-GC ve GPU Framebuffer İzolasyonu (Zero-Drop 60 FPS Invariance):**
    - **Pahalı GPU `saveLayer` Kesin Yasağı:** Gece tülü, sis, atmosferik karartma veya fener efektlerinde `canvas.saveLayer(bounds, Paint())` kullanımı KESİNLİKLE YASAKTIR. `saveLayer`, GPU üzerinde tam ekran boyutunda offscreen doku tahsis ederek kare hızını 20 FPS'e düşürür. Tüm ışıklandırma ve karartma efektleri doğrudan ana hedef yüzeye `BlendMode.screen`, `BlendMode.plus` veya `BlendMode.colorDodge` harmanlama modları ve `RadialGradient` / `LinearGradient` ile çizilmelidir.
    - **Render & Update Döngülerinde Sıfır Tahsis Değişmezi:** `render()` ve `update()` döngüleri içinde asla yeni `Path()`, `Paint()`, `List` veya `LightEmitter` nesnesi üretilemez (`new`lenemez). Işık kaynakları (`LightEmitter`) her karede değil, yalnızca binalar veya gün/gece geçişi tetiklendiğinde güncellenen önceden tahsis edilmiş bir havuzda (`_cachedLightEmitters`) tutulmalıdır. Voksel ışık hüzmeleri için sınıf seviyesinde `static final Path _sharedRayPath = Path();` tanımlanmalı ve her huzmede `_sharedRayPath.reset()` kullanılmalıdır.
    - **Flutter UI Katmanında Rebuild ve Ağır Matematik İzolasyonu:** Saniyelik pasif kaynak sayaç artışlarında (altın, odun artışı) tüm haritayı tarayan ağır fonksiyonlar (`calculateNetRates`, sinerji döngüleri vb.) doğrudan `build()` içinde KESİNLİKLE ÇALIŞTIRILAMAZ. Bu fonksiyonlar `ratesHash` (karo sayısı, kale seviyesi, taç, mevsim, frenzy) mekanizmasıyla önbelleklenmeli veya state notifier içinde yönetilmelidir. `TileActionSheet` gibi paneller boştayken (`selectedCoord == null` ve `isDismissed`), `ref.watch(gameStateProvider)` ile saniyelik sayaç artışlarında gereksiz `build()` tetiklememelidir (`const SizedBox.shrink()` ile dinleme kesilmelidir).
    - **Harita Motoru Görsel Eşitlik Koruması (`_isTileVisuallyIdentical`):** Karoların ekonomik sayaçları (`accumulatedResource`, `soilHealth` vb.) arttığında harita voksel görseli değişmez. Harita bileşeni (`HexMapGame._updateTiles`), `_isTileVisuallyIdentical` kontrolüyle görseli değişmeyen karolar için 6 yönlü komşu ve kot farkı matris hesaplamalarını atlamak zorundadır (`.agents/rules/organik_desenkronizasyon_ve_zero_gc_render_standartlari.md`).
